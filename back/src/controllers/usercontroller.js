...
export const createUser = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ res: newUser, message: "User successfully created" });
  } catch (err) {
...
export const userDashboard = async (req, res) => {
  ...
  try {
    ...
    const expenses = transactionType.filter(
      (t) => t.TransactionType == "Ingreso"
    );
    const incomes = transactionType.filter(
      (t) => t.TransactionType == "Egreso"
    );

    const expensesAmount = expenses.reduce(
      (acc, t) => acc + parseInt(t.Amount),
      0
    );
