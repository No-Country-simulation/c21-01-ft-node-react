  ...
  return (
    <div className="w-full px-4">
      ...
      <div className="flex justify-center space-x-1 mb-6">
        <button
          ...
          className={`w-button-xs flex justify-center items-center text-center border border-primary text-primary py-buttonPadding rounded-3xl hover:bg-primary-dark hover:text-white hover:border-primary-dark transition duration-300 ${
            activeTab === "ingresos" ? "bg-cyan-700 text-white" : "bg-white text-black"
          }`}
        >
          Ingresos
        </button>
        <button
          ...
          className={`w-button-xs flex justify-center items-center text-center border border-primary text-primary py-buttonPadding rounded-3xl hover:bg-primary-dark hover:text-white hover:border-primary-dark transition duration-300 ${
            activeTab === "egresos" ? "bg-cyan-700 text-white" : "bg-white text-black"
          }`}
        >
          Egresos
        </button>
      ...
      <div className="bg-white p-4 rounded-lg shadow mt-4">
        {activeTab === "ingresos" ? (
          <div>
            <h2 className="text-xl font-semibold mb-4 text-green-600">Listado de Ingresos</h2>
            <ul>
              {ingresos.map((ingreso) => (
                <li key={ingreso.id} className="border-b py-2 flex justify-between">
                  <span>{ingreso.descripcion}</span>
                  <span className="font-semibold text-green-600">${ingreso.monto}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <h2 className="text-xl font-semibold mb-4 text-red-600">Listado de Egresos</h2>
            <ul>
              {egresos.map((egreso) => (
                <li key={egreso.id} className="border-b py-2 flex justify-between">
                  <span>{egreso.descripcion}</span>
                  <span className="font-semibold text-red-600">${egreso.monto}</span>
                </li>
              ))}
            </ul>
          </div>
